...
/*const int THREADS_NUMBER = 10;   //кол-во параллельных процессов
...
void PrepareParallelCountingSort(int[] inputArray)
{
    ...
    // ждем выполнения потоков до конца 
    foreach (var thread in threadParallel)
    {
        thread.Join();
    }
...
bool EqualityMatrix(int[] fmatrix, int[] smatrix)
{
    for (int i = 0; i < N; i++)
    {
        if (fmatrix[i] != smatrix[i]) return false;
    }
    return true;
}
...
//!!! Это код авторского репозитория https://github.com/r0acho/CountingSort/tree/parallel
//!!! Несмотря на коммент //lock в функции CountingSortParallel, всегда выводит TRUE !!!!
